...
int Relation::GetNumOfTuples(Relation r) {
   int size = 0;
...
Relation* Relation::SingleIDSelect(Relation r, int firstIndx, int secondIndx, bool rules) { //BAD FOR DEAVOO
   Relation* newSelectR = new Relation();
   ...
   for (iter = r.rows.begin(); iter != r.rows.end(); iter++) {
      std::vector<std::string> vex = *iter;
      if ((vex.at(firstIndx) != vex.at(secondIndx)) || rules) {
         newSelectR->rows.insert(vex);
      }
   }
   return newSelectR;
...
Relation* Relation::Union(Relation one, Relation two) {
   ...
   newUnionR->rName = two.rName; //What should I name this??
   
   for (unsigned int i = 0; i < two.schema.size(); i++) {
      newUnionR->schema.push_back(two.schema.at(i));
      for (unsigned int j = 0; j < one.schema.size(); j++) {
         if (two.schema.at(i) == one.schema.at(j)) {
            break; //found a match don't need to keep looking to rename
         }
         else {
            Rename(one, newUnionR->schema);
         }
      }
   }
