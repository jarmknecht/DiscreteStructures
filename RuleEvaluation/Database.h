...
class Database {
   public:
      ...
      void ClearOldFacts();
      void lookAtRules(std::vector<Rule*> rs);
      int GetSize();
      ...
      void evalRulesNewFacts(Relation* one, Relation* two, Relation* three, Relation* projectR, Relation* unionR, Relation* toUnion, unsigned int i, std::vector<Rule *> rs);
      ...
      Relation* CheckIfUnionIsNull(Relation* toUnion, unsigned int i, std::vector<Rule *> rs);
