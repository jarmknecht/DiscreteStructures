...
void Database::lookAtRules(std::vector<Rule *> rs) {
   bool keepLooking = true;
   int sizeOfRelationBefore = 0;
   int sizeOfRelationAfter = 0;
   
   Graph myGraph = Graph(rs);
   std::vector<std::vector<Rule*>> connections = myGraph.StrongConnections(); //strong connections are giving 1 reflexive bool true because of the children of it 
   
   std::cout << std::endl << "Rule Evaluation" << std::endl;
   
   for (unsigned int i = 0; i < connections.size(); i ++) {
      std::vector<Rule*> connectionSubset = connections.at(i);
      
      int numOfPasses = 0;
      
      while (keepLooking) {
         sizeOfRelationBefore = GetSize();
         evalRules(connectionSubset);
         numOfPasses++;
         sizeOfRelationAfter = GetSize();
         
         /*std::cout << "sizeBefore = sizeAfter";
         ...
         std::cout << "connectionSubset size";
         
         if (connectionSubset.size() == 1) {
            std::cout << " = true is 1\n";
         }
         else {
            std::cout << " = false is not 1\n";
         }
         
         std::cout << "Isn't reflexive";
         
         if (!connectionSubset.at(0)->reflexive) {
            std::cout << " = true\n";
         }
         ...
         std::cout << "Combo of size 1 and not reflexive";
         
         if (connectionSubset.size() == 1 && !connectionSubset.at(0)->reflexive) {
            std::cout << " = true\n";
         }
         else {
            std::cout << " = false\n";
         ...
         if ((sizeOfRelationBefore == sizeOfRelationAfter) || (connectionSubset.size() == 1 && !connectionSubset.at(0)->reflexive)) {
            break;
         }
      }
      std::cout << numOfPasses << " passes: ";
      for (unsigned int j = 0; j < connectionSubset.size(); j++) {
         if (j > 0) {
            std::cout << ",";
         }
         std::cout << connectionSubset.at(j)->ID;
      }
      std::cout << std::endl;
   }
...
void Database::evalRules(std::vector<Rule *> rs) {
   ...
   for (unsigned int i = 0; i < rs.size(); i++) {
      //std::cout << "Calling evalRule on " << rs.at(i)->ID << std::endl;
      ...
      if (newFacts.size() < 1) {
         ...
         evalRulesNewFacts(one, two, three, projectR, unionR, toUnion, i, rs);
      }
   }
}

Relation* Database::CheckIfUnionIsNull(Relation* toUnion, unsigned int i, std::vector<Rule *> rs) {
   if (toUnion == NULL) {
      Relation* newUnion = new Relation();
...
void Database::evalRulesNewFacts(Relation* one, Relation* two, Relation* three, Relation* projectR, Relation* unionR, Relation* toUnion, unsigned int i, std::vector<Rule *> rs) {
   std::vector<int> indexes;
   ...
   for (unsigned int j = 0; j < newFacts.size(); j++) {
      ...
      if (one == NULL) {
         if (j < newFacts.size() - 1) {
            ...
            if (j == newFacts.size() - 1) {
               indexes = GetProjCols(one, rs.at(i));
               projectR = one->Project(*one, indexes);
               toUnion = Find(rs.at(i)->myHeadPredicate->leftId);
               toUnion = CheckIfUnionIsNull(toUnion, i, rs);
               unionR = projectR->Union(*projectR, *toUnion);
               delete toUnion;
            ...
            indexes = GetProjCols(two, rs.at(i));
